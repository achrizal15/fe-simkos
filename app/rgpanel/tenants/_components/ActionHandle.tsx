        ...
        <div className="gap-2 flex justify-center items-center">
           ...
           <PrimeButton tooltip="Edit" tooltipOptions={{ showDelay: 500, position: 'bottom' }} severity="warning" icon="pi pi-pencil"></PrimeButton>
            ...
            {item.deleted_at != null
                ? <Restore item={item} queryKey={queryKey} toastMessage={(message) => toast.current?.show(message)} />
                : <Delete queryKey={queryKey} item={item} toastMessage={(message) => toast.current?.show(message)} />
            }
        </div>
    )
}
